...
CREATE TABLE IF NOT EXISTS procedimentos (
    ...
    instrumentos TEXT,
    precisao INTEGER,
    tempo_pontuacao INTEGER,
    uso_instrumentos INTEGER,
    geral REAL,
    falhas TEXT,
    notas TEXT
)
...
# Inserção dos dados
for dado in dados_sql:
    try:
        cursor.execute('''
        INSERT INTO procedimentos (procedimento, nivel, data, tempo, instrumentos,
                                   precisao, tempo_pontuacao, uso_instrumentos, geral,
                                   falhas, notas)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            dado['procedimento'],
            ...
            dado['data'],
            dado['tempo'],
            ', '.join(dado['instrumentos']),
            dado['pontuacao']['precisao'],  # Ajustado para acessar a pontuação corretamente
            dado['pontuacao']['tempo'],      # Ajustado para acessar a pontuação corretamente
            dado['pontuacao']['uso_instrumentos'],  # Ajustado para acessar a pontuação corretamente
            dado['pontuacao']['geral'],      # Ajustado para acessar a pontuação corretamente
            ', '.join(dado['falhas']),
            dado['notas']
        ))
